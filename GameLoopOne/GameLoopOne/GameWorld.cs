...
namespace GameLoopOne
{
    ...
    class GameWorld
    {
        ...
        public static void SetupDifferentWorlds()
        {
            foreach (GameObject go in objects.ToList())
            {
                if (!(go is Player || go is Sky || go is Weapon))//dont remove the player or the sky
                {
                    removeList.Add(go);
            ...
            switch (iLevel)
            {
                case 0:
                    ...
                    objects.Add(new Enemy("player/sprites/playersprite1.png", new Vector2D(770, 590), .75f, (new Shotgun(new Vector2D(770, 590), 1))));
